...
export default function ResetPasswordForm({ onSignUpClick }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(resetPassword({ token, newPassword: values.newPassword, navigate })); // Pass token and navigate with new password
    },
  });
  ...
  return (
    <ModalWindow open={true}>
      <FormContainer>
        ...
        <SocialIcons>
          <Icon>
            <FaDiscord style={{ height: '40px', width: '40px', fill: '#fff' }} />
          </Icon>
          <Icon>
            <FaLinkedin style={{ height: '40px', width: '40px', fill: '#fff' }} />
          </Icon>
          <Icon>
            <FaTwitterSquare style={{ height: '40px', width: '40px', fill: '#fff' }} />
          </Icon>
        </SocialIcons>
        <SignUp>
          Don’t have an account?
          <Link onClick={onSignUpClick} style={{ color: '#ffff', marginLeft: '5px' }}>
            Sign Up
          </Link>
        </SignUp>
      </FormContainer>

...
const ProductPage = () => {
  ...
  return (
    <>
    {status == 'loading' ? <><Box component="div" className={styles.productContainer}>Loading...</Box></> : <><Container sx={{width: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', margin: '20px 0', gap: '20px', fontFamily: '"Gill Sans", sans-serif'}}>
      <Box component="div" sx={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: '50px'}}>
        <Box component="div" sx={{marginTop: '25px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', textAlign: 'center', position: 'relative', width: '60%'}}>
          <Button variant='contained' disableElevation sx={{position: 'absolute', width: '30%', borderRadius: '0'}}>Sale</Button>
          <img src={product?.imageUrls} alt={product.name} className={styles.productImg}/>
        ...
        <Box component="div" sx={{textAlign: 'left'}}>
           ...
           <Box component="div" sx={{display: 'flex', justifyContent: 'flex-start', gap: '20px'}}>
             {!favorite && <AiOutlineHeart onClick={handleFavorite} style={{height: '25px', width: 'auto', color: '#828386'}}/>}
             {favorite && <AiFillHeart onClick={handleFavorite} style={{height: '25px', width: 'auto', color: '#828386'}}/>}
